...
class AllocationSnapshotCollector
{
    ...
    public function collect(?CarbonImmutable $collectedAt = null): int
    {
        ...
        return $this->collectAt($snapshotAt);
    }

    /**
     * Rebuild the available half-hour snapshots for one airport and one UTC day.
     */
    public function rebuildDay(
        CarbonImmutable $snapshotDate,
        int $airportId,
        ?CarbonImmutable $rebuiltAt = null,
    ): int {
        $dayStart = $snapshotDate->utc()->startOfDay();
        $rebuildTime = ($rebuiltAt ?? CarbonImmutable::now('UTC'))->utc()->startOfMinute();
        $lastSnapshotAt = $dayStart->addDay()->subMinutes(30);

        if ($dayStart->toDateString() === $rebuildTime->subDay()->toDateString()) {
            $lastSnapshotAt = $dayStart
                ->setHour($rebuildTime->hour)
                ->setMinute(intdiv($rebuildTime->minute, 30) * 30);
        }

        $intervalCount = intdiv($dayStart->diffInMinutes($lastSnapshotAt), 30);
        $snapshotCount = 0;

        foreach (range(0, $intervalCount) as $interval) {
            $snapshotCount += $this->collectAt($dayStart->addMinutes($interval * 30), $airportId);
        }

        DB::table('airport_allocation_snapshots')
            ->where('airport_id', $airportId)
            ->where('snapshot_date', $dayStart->toDateString())
            ->where('captured_at', '>', $lastSnapshotAt)
            ->delete();
    ...
    /**
     * @param  int|null  $airportId  Limit collection to one managed airport when provided.
     */
    private function collectAt(CarbonImmutable $snapshotAt, ?int $airportId = null): int
    {
        $snapshotAt = $snapshotAt->utc()->startOfMinute();
        ...
        if ($airportId !== null) {
            $airports->whereKey($airportId);
        }

        $airports->each(function (Airport $airport) use (
            &$snapshots,
            $snapshotAt,
            $snapshotDate,
            $dayStart,
            ...
            $flightCounts = Flight::query()
                ...
                ->where('flights.estimated_departure_at', '<=', $snapshotAt)
                ->selectRaw(
